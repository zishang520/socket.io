...
func TestWireEncodersReturnReaderErrors(t *testing.T) {
	for name, encode := range map[string]func(*failingWireReader) error{
		"packet-json": func(r *failingWireReader) error {
			_, err := json.Marshal(&ValkeyPacket{Packet: &parser.Packet{Type: parser.EVENT, Data: []any{"event", r}}})
			return err
		},
		"packet-msgpack": func(r *failingWireReader) error {
			_, err := msgpack.Marshal(&ValkeyPacket{Packet: &parser.Packet{Type: parser.EVENT, Data: []any{"event", r}}})
			return err
		},
		"request-json": func(r *failingWireReader) error {
			_, err := json.Marshal(&ValkeyRequest{Type: SERVER_SIDE_EMIT, Data: []any{"event", r}})
			return err
		},
		"request-msgpack": func(r *failingWireReader) error {
			_, err := msgpack.Marshal(&ValkeyRequest{Type: SERVER_SIDE_EMIT, Data: []any{"event", r}})
			return err
		},
		"response-json": func(r *failingWireReader) error {
			_, err := json.Marshal(&ValkeyResponse{Packet: map[string]any{"data": r}})
			return err
		},
